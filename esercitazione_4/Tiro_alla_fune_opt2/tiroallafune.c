...
void TiroAllaFune(int* set, int* vcurr, int* vbest, int* set_check, int i, int n, int k, int sum1, int* sum_tot, int* diff_best, int* calls) {
	(*calls)++;
	for (int j = k; j < n; ++j) {
		if (set_check[j] == 0) {
			vcurr[i] = set[j];
			...
			if (i < n / 2 - 1) {
				TiroAllaFune(set, vcurr, vbest, set_check, i + 1, n, j + 1, sum1, sum_tot, diff_best, calls);
			}
			else {
...
int main(int argc, char** argv) {
	...
	TiroAllaFune(set, vcurr, vbest, set_check, i, n, k, sum1, &sum_tot, &diff_best, &calls);
